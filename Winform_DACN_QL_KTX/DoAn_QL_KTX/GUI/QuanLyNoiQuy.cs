...
namespace GUI
{
    public partial class QuanLyNoiQuy : Form
    {
        ...
        {
            string maSinhVien = txt_manv_lapvp.Text; // Lấy mã sinh viên từ TextBox
            ...
            if (!string.IsNullOrEmpty(maSinhVien)) // Kiểm tra nếu mã sinh viên không trống
            {
                ...
                if (tenSinhVien != null)
                {
                    txt_manv_lapvp.Text = tenSinhVien; // Hiển thị tên sinh viên vào TextBox
                }
                else
                {
                    txt_manv_lapvp.Text = "Không tìm thấy sinh viên"; // Thông báo nếu không tìm thấy sinh viên
                }
            }
            else
            {
                txt_manv_lapvp.Text = ""; // Nếu mã sinh viên trống, xóa tên sinh viên
            }
        }
            ...
            try
            {
                ...
                if (e.RowIndex >= 0)
                {
                    ...
                    if (viPham != null)
                    {
                        ...
                        txt_manv_lapvp.Text = viPham.MaNhanVien;
